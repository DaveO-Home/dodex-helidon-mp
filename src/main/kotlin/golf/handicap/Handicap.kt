...
class Handicap {
    ...
    fun getHandicap(golfer: Golfer, entityManager: EntityManager): MutableMap<String, Any> {
        ...
        try {
            ...
            if ((handicapData["array"] as JsonArray).isEmpty()) {
                latestTee["handicap"] = 0.0f
            } else {
                val objects = (handicapData["array"] as JsonArray).iterator()
                var i = 0
                ...
                golfer.handicap = handicap.toFloat().toDouble()


                transaction.begin()

                val scoresUpdatedCount = golferScores.setGolferHandicap(golfer, em)
